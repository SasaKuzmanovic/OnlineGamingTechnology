...
bool Server::ListenForNewConnection()
{
	...
	else //If client connection properly accepted
	{
		...
		if (player1Position.empty())
		{
			player1Position = "No position yet";
		}
		SendPlayerPosition(TotalConnections, player1Position);
...
bool Server::ProcessPacket(int ID, Packet _packettype)
{
	...
	case PlayerPos:
	{
		...
		for (int i = 0; i < TotalConnections; i++)
		{
			if (i == ID) //If connection is the user who sent the message...
				continue;//Skip to the next user since there is no purpose in sending the message back to the user who sent it.
			if (!SendPlayerPosition(i, Message)) //Send message to connection at index i, if message fails to be sent...
			{
				std::cout << "Failed to send message from client ID: " << ID << " to client ID: " << i << std::endl;
			}
		}
		
		player1Position = Message;

		std::cout << player1Position << std::endl;

		std::cout << "Processed chat message packet from user ID: " << ID << Message << std::endl;
		break;
	...
	case PositionRequest:
	{
		...
		SendPlayerPosition(ID, player1Position);
		std::cout << "Processed chat message packet from user ID: " << ID << Message << std::endl;
		break;
